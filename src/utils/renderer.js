...
const AUDIO_CHUNK_DURATION = 0.04; // 40ms chunks for lower latency
const BUFFER_SIZE = 1024; // Smaller buffer for ~42ms callback intervals
        ...
        async blob => {
            ...
            reader.onloadend = async () => {
                ...
                if (result.success) {
                    ...
                    assistant.addNewResponse(`Error: ${result.error}`);
                }
            };
            reader.readAsDataURL(blob);
        },
...
function handleShortcut(shortcutKey) {
    const currentView = assistant.getCurrentView();

    if (shortcutKey === 'ctrl+enter' || shortcutKey === 'cmd+enter') {
        if (currentView === 'main') {
            assistant.element().handleStart();
        } else {
...
const assistantApp = document.querySelector('assistant-app');
...
const assistant = {
    ...
    getLayoutMode: () => assistantApp.getLayoutMode(),

    // Status and response functions
    setStatus: text => assistantApp.setStatus(text),
    addNewResponse: response => assistantApp.addNewResponse(response),
    updateCurrentResponse: response => assistantApp.updateCurrentResponse(response),
